...
import { getCategoryById, getParentCategories, updateCategory, CategoryFormData } from "@/app/actions/categories.action"
...
export default function EditCategoryPage() {
  ...
  // Helper function to safely check if a category is a parent category
  const isCategoryParent = (category: any, categories: any[]) => {
    if (!categories || !category) return false;
    
    // Check if any category has this one as parent
    ...
    async function fetchCategoryData() {
      try {
        ...
        if (result.success && result.data) {
          ...
          toast.error(result.error || "Failed to load category data")
          router.push("/admin/categories")
        }
      } catch (error) {
  ...
  useEffect(() => {
    async function fetchParentCategories() {
      ...
      try {
        ...
        if (result.success && result.data) {
          // Filter out this category and its children to prevent circular references
          const filteredParents = result.data.filter(
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Include existing image if using the original
      if (originalImage && previewUrl === originalImage) {
        formData.existingImage = originalImage
      }
    ...
    <div className="space-y-6">
      ...
      <Card>
        <CardHeader>
          ...
          <CardDescription>Edit your category details</CardDescription>
        </CardHeader>
        <CardContent className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <p className="text-sm text-muted-foreground">
              Used in URLs: /products/category/{categoryData.slug || "example-slug"}
            </p>
          </div>
